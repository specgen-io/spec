...
`
	...
	assert.Equal(t, len(http.Versions), 1)
	assert.Equal(t, len(http.Versions[0].Apis), 1)
	api := http.Versions[0].Apis[0]
...
`
	...
	assert.Equal(t, len(http.Versions), 2)

	version0 := http.Versions[0]
	assert.Equal(t, version0.Version.Source, "v2")
	assert.Equal(t, version0.GetUrl(), "/v2")
	assert.Equal(t, len(version0.Apis), 1)
	api_v2 := version0.Apis[0]
	assert.Equal(t, api_v2.Name.Source, "test")
	assert.Equal(t, len(api_v2.Operations), 1)

	version1 := http.Versions[1]
	assert.Equal(t, version1.Version.Source, "")
	assert.Equal(t, version1.GetUrl(), "")
	assert.Equal(t, len(version1.Apis), 1)
	api := version1.Apis[0]
	assert.Equal(t, api.Name.Source, "test")
...
`
	...
	version0 := http.Versions[0]
	assert.Equal(t, version0.Version.Source, "v2")
	assert.Equal(t, version0.GetUrl(), "/version2")

	version1 := http.Versions[1]
	assert.Equal(t, version1.Version.Source, "")
	assert.Equal(t, version1.GetUrl(), "/default")
}
